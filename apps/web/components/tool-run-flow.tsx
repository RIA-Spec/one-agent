...
import { useExecutionProgress, type StepState } from '@/hooks/use-execution-progress';
...
function FlowNode({ data }: { data: NodeData }) {
  ...
  const getBgColor = () => {
    if (data.status === 'error') return 'bg-red-50 border-red-300';
    if (data.status === 'running') return 'bg-blue-50 border-blue-300 shadow-md';
    if (data.status === 'completed') {
      switch (data.type) {
        case 'act': return 'bg-blue-50 border-blue-300';
        case 'reason': return 'bg-orange-50 border-orange-300';
        case 'end': return 'bg-green-50 border-green-300';
  ...
  return (
    ...
    >
      <Handle type="target" position={Position.Left} className="!h-1.5 !w-1.5 !bg-transparent !border-transparent" />
      ...
      {data.context && data.context.length > 0 && (
        <div className="flex flex-wrap items-center gap-0.5 mt-0.5">
          {data.context.map((ctx, i) => {
            ...
            const color = ctx.type === 'loop'
              ? 'bg-violet-50 text-violet-600 border-violet-200'
              : ctx.type === 'condition'
                ? 'bg-amber-50 text-amber-600 border-amber-200'
                : 'bg-rose-50 text-rose-500 border-rose-200';
            return (
              <span key={i} className={`inline-flex items-center gap-0.5 px-1 py-px rounded border text-[8px] font-medium leading-tight ${color}`}>
                {icon}
      ...
      <Handle type="source" position={Position.Right} className="!h-1.5 !w-1.5 !bg-transparent !border-transparent" />
    </div>
  );
}
...
function flattenTree(
  ...
  for (const step of steps) {
    if (step.type === 'act' || step.type === 'reason') {
      result.push({ step, trackableIndex: counter.value++, context: ancestors });
    } else if (step.children) {
      const crumb: ContextCrumb = { type: step.type, label: shortenContext(step.name) };
      result.push(...flattenTree(step.children, [...ancestors, crumb], counter));
    }
  }
  return result;
}
...
function countTrackable(steps: ASTStep[]): number {
  ...
  for (const s of steps) {
    if (s.type === 'act' || s.type === 'reason') c++;
    if (s.children) c += countTrackable(s.children);
  }
  return c;
}
...
function parsePyBlock(
  ...
  while (i < lines.length) {
    ...
    // for / while loop
    if ((/^for\s+.+\s+in\s+/.test(t) || /^while\s+/.test(t)) && t.endsWith(':')) {
      const { steps: ch, endIdx } = parsePyBlock(lines, i + 1, indent);
      steps.push({ type: 'loop', name: t.slice(0, -1).trim(), args: [], line: i + 1, ...(ch.length > 0 && { children: ch }) });
      i = endIdx; continue;
    }
    ...
    if (/^(?:if|elif)\s+.+:|^else\s*:/.test(t)) {
      const { steps: ch, endIdx } = parsePyBlock(lines, i + 1, indent);
      steps.push({ type: 'condition', name: t.slice(0, -1).trim(), args: [], line: i + 1, ...(ch.length > 0 && { children: ch }) });
      i = endIdx; continue;
    }
    ...
    if (/^try\s*:|^except.*:|^finally\s*:/.test(t)) {
      const { steps: ch, endIdx } = parsePyBlock(lines, i + 1, indent);
      steps.push({ type: 'error-handling', name: t.slice(0, -1).trim(), args: [], line: i + 1, ...(ch.length > 0 && { children: ch }) });
      i = endIdx; continue;
    }

    // async def / def / class — transparent
    if ((/^(?:async\s+)?def\s+/.test(t) || /^class\s+/.test(t)) && t.endsWith(':')) {
    ...
    if (/await\s+(act|reason)\s*\(/.test(t)) {
      ...
      if (fnType === 'act') {
        ...
        const rest = (argsStr || '').substring(nameMatch ? nameMatch[0].length : 0).replace(/^\s*,\s*/, '').trim();
        const preview = rest.replace(/\s+/g, ' ').replace(/^['"]|['"]$/g, '').substring(0, 60);
        steps.push({ type: 'act', name, args: preview ? [preview] : [], line: i + 1 });
      } else {
        const fstr = argsStr?.match(/^f['"](.{0,60}?)['"]/);
        const str = argsStr?.match(/^['"](.{0,60}?)['"]/);
        let prompt = 'Analyze';
        if (fstr) prompt = 'f"' + fstr[1].replace(/\{[^}]*\}/g, '{...}').trim() + '"';
        else if (str) prompt = str[1].trim();
...
function collectPyCallText(lines: string[], idx: number): { text: string; endLine: number } {
  let text = '', depth = 0, started = false, inStr: string | null = null;
...
function extractCallArgs(callText: string): string | null {
  ...
  if (!m) return null;
  const start = callText.indexOf('(', m.index!) + 1;
  let depth = 1, i = start, inSQ = false, inDQ = false;
  while (i < callText.length && depth > 0) {
    if (callText[i] === '\\' && (inSQ || inDQ)) { i += 2; continue; }
    if (callText[i] === "'" && !inDQ) { inSQ = !inSQ; i++; continue; }
    if (callText[i] === '"' && !inSQ) { inDQ = !inDQ; i++; continue; }
    if (!inSQ && !inDQ) { if (callText[i] === '(') depth++; else if (callText[i] === ')') depth--; }
...
function parseBashTree(code: string): ASTStep[] {
  ...
  for (let li = 0; li < lines.length; li++) {
    ...
    if (/^elif\s+/.test(t) || t === 'else') {
      if (stack.length > 0 && stack[stack.length - 1].node.type === 'condition') stack.pop();
      const name = t === 'else' ? 'else' : t.replace(/;\s*then\s*$/, '').replace(/\s+then\s*$/, '').trim();
      const node: ASTStep = { type: 'condition', name, args: [], line: ln, children: [] };
      target().push(node);
    ...
    if (stack.length > 0 && stack[stack.length - 1].closeRe.test(t)) { stack.pop(); continue; }

    // for / while
    if (/^(?:for|while)\s+/.test(t)) {
      const name = t.replace(/;\s*do\s*$/, '').replace(/\s+do\s*$/, '').trim();
      const node: ASTStep = { type: 'loop', name, args: [], line: ln, children: [] };
      target().push(node);
      if (/\bdo\s*$/.test(t)) stack.push({ node, closeRe: /^done\b/ });
      continue;
    }
    ...
    // if
    if (/^if\s+/.test(t)) {
      const name = t.replace(/;\s*then\s*$/, '').replace(/\s+then\s*$/, '').trim();
      const node: ASTStep = { type: 'condition', name, args: [], line: ln, children: [] };
      target().push(node);
      if (/\bthen\s*$/.test(t)) stack.push({ node, closeRe: /^(?:elif|else|fi)\b/ });
      continue;
    }
    ...
    while ((cm = cmdRe.exec(t)) !== null) {
      ...
      if (cmdType === 'act') {
        ...
        const p = bashFlags(cmdStr, '--prompt').filter(x => x !== '-').join(' ').substring(0, 60);
        target().push({ type: 'act', name, args: p ? [p] : [], line: ln });
      } else {
        const p = bashFlags(cmdStr, '--prompt').filter(x => x !== '-').join(' ').substring(0, 60);
        target().push({ type: 'reason', name: p || 'Analyze', args: [], line: ln });
      }
    }
  }
  return root;
...
function extractBashCmd(line: string, start: number): string {
  let i = start, inSQ = false, inDQ = false, depth = 0;
  while (i < line.length) {
    const ch = line[i];
    if (ch === '\\' && inDQ) { i += 2; continue; }
    ...
    if (!inSQ && !inDQ) {
      ...
      if (depth === 0 && (ch === '|' || ch === ';' || ch === ')' || (ch === '&' && line[i + 1] === '&'))) break;
    }
    i++;
  }
...
function bashFlags(cmd: string, flag: string): string[] {
  ...
  const re = new RegExp(`${flag.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}\\s+`, 'g');
  let m: RegExpExecArray | null;
  while ((m = re.exec(cmd)) !== null) {
    const vs = m.index + m[0].length;
    if (vs >= cmd.length) continue;
    const ch = cmd[vs];
    if (ch === '"' || ch === "'") {
      let j = vs + 1, v = '';
      while (j < cmd.length && cmd[j] !== ch) {
        if (cmd[j] === '\\' && ch === '"') { v += cmd[j + 1] || ''; j += 2; continue; }
        v += cmd[j]; j++;
      }
      results.push(v);
    } else {
      const w = cmd.substring(vs).match(/^(\S+)/);
      if (w) results.push(w[1]);
    }
  }
  return results;
}
 ...
 */
function deriveStepStates(trackableCount: number, overallState: ExecutionState, isError?: boolean): StepState[] {
  return Array.from({ length: trackableCount }, () => {
    if (overallState === 'available') return { status: isError ? 'error' as const : 'completed' as const };
    if (overallState === 'streaming') return { status: 'running' as const };
...
function buildNodesFromFlat(
  ...
  flatNodes.forEach((fn, idx) => {
    ...
    const details = fn.step.type === 'act'
      ? (fn.step.args.length > 0 ? fn.step.args.join(', ').substring(0, 50) : '')
      : fn.step.type === 'reason'
        ? (fn.step.name ? fn.step.name.substring(0, 50) : '')
        : '';
  ...
  // End node
  xPos += xSpacing;
  const allCompleted = flatNodes.length > 0 && flatNodes.every(fn => {
    const s = stepStates[fn.trackableIndex];
    return s?.status === 'completed' || s?.status === 'error';
  });
  const anyError = flatNodes.some(fn => stepStates[fn.trackableIndex]?.status === 'error');
...
function buildEdgesFromFlat(
  ...
  const getNodeStatus = (idx: number): StepState['status'] => {
    if (idx < 0 || idx >= flatNodes.length) return 'pending';
  ...
  // last node → end
  const endColor = overallState === 'available'
...
export function ToolRunFlow({ state, mode = 'python', code = '', output, isError }: ToolRunFlowProps) {
  const { progress } = useExecutionProgress();

  // Get tree-structured steps: prefer progress data, fallback to code parsing
  const steps = useMemo(() => {
    if (progress?.steps && progress.steps.length > 0) return progress.steps;
  ...
  const stepStates = useMemo(() => {
    if (progress && progress.stepStates.length === trackableCount && trackableCount > 0) {
      return progress.stepStates;
    }
    return deriveStepStates(trackableCount, state, isError);
  }, [progress?.stepStates, trackableCount, state, isError]);
