...
> **Authors**: Anthropic Applied AI team (Prithvi Rajasekaran, Ethan Dixon, Carly Ryan, Jeremy Hadfield, et al.)  
...
Since we wrote [Building effective AI agents](https://www.anthropic.com/engineering/building-effective-agents), we've gravitated towards a simple definition for agents: **LLMs autonomously using tools in a loop**.
...
After context resets, the agent reads its own notes and continues multi-hour sequences. This coherence across summarization steps enables long-horizon strategies that would be impossible when keeping all the information in the LLM's context window alone.
